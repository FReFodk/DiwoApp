...
const checkMessage = async function() {
  ...
  let response = await fetch(
    'http://diwo.nu/public/api/getLastMessageReadStatus',
    {
      method: 'POST',
      headers: headers,
    },
...
const checkSocialKapital = async function() {
  let token_value = await AsyncStorage.getItem('usertoken');
  let headers = new Headers();
  let auth;
  if (!token_value) {
  ...
  let response = await fetch(
    'http://diwo.nu/public/api/lastAddedSocialkapital',
    {
      method: 'POST',
      headers: headers,
    },
  ...
  if (responseJson.status == 200) {
    if (responseJson.kapital_data[0]) {
      ...
      var now = new Date();
      let lastDate = getDaysInMonth(now.getMonth() + 1, now.getFullYear()) - 3;
      let activationDate =
      ...
      var now = new Date();
      let PrevlastDate = getDaysInMonth(now.getMonth(), now.getFullYear()) - 3;
...
const checkWorkJoy = async function() {
  let token_value = await AsyncStorage.getItem('usertoken');
  let headers = new Headers();
  let auth;
  if (!token_value) {
  ...
  headers.append('Authorization', auth);
  let rturn = false;
  let response = await fetch('http://diwo.nu/public/api/lastAddedWorkJoy', {
    method: 'POST',
    headers: headers,
  });
  let responseJson = response.json();
...
BackgroundTask.define(async () => {
  ...
  if (rturn) count++;
  rturn = false;
  response = await fetch('http://diwo.nu/public/api/lastAddedSocialkapital', {
    method: 'POST',
    headers: headers,
  });
  ...
  if (rturn) count++;
  rturn = false;
  response = await fetch('http://diwo.nu/public/api/lastAddedWorkJoy', {
    method: 'POST',
    headers: headers,
  });
  responseJson = response.json();
