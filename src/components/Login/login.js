...
import { StyleSheet, Text, View, TouchableOpacity, Image, KeyboardAvoidingView, TextInput, Dimensions, ActivityIndicator, Linking, BackHandler } from 'react-native';
        ...
        <View style={styles.container}>
          <Image
            style={{
              ...
              bottom: -width * 0.3, right: -width * 0.2, opacity: 0.1, transform: [{ rotate: "320deg" }]
            }}
          ...
          <View style={styles.first_container}>
            <Image
              style={{ width: width > height ? wp('30%') : wp('70%'), height: width > height ? wp('15%') : wp('40%') }}
            ...
            <KeyboardAvoidingView >
              ...
              <View>
                <Text style={{ fontSize: width > height ? wp('2%') : wp('3.5%'), position: 'absolute', left: width > height ? wp('33%') : wp('16%'), backgroundColor: 'white', zIndex: 999, top: 5 }}> {Text_EN.Text_en.password} </Text>
                <TextInput
                  style={styles.input}
                  autoCapitalize="none"
                  returnKeyLabel="Go"
                ...
                <TouchableOpacity style={{ marginLeft: width > height ? wp('62%') : wp('63%'), marginRight: width > height ? wp('30%') : wp('12%') }} onPress={() => navigate('Forgot')}>
                  <Text style={{ textAlign: 'right', marginRight: width > height ? wp('1%') : wp('2%'), fontSize: width > height ? wp('2%') : wp('3.5%') }}>{Text_EN.Text_en.forget_password}</Text>
                </TouchableOpacity>
              ...
              <View>
                <LinearGradient colors={['#87d9f7', '#00a1ff']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.log_btn}>
                  <TouchableOpacity onPress={() => this.login_clicked()}>
                    <Text style={{ textAlign: 'center', color: 'white', fontSize: width > height ? wp('2%') : wp('4%'), fontWeight: 'bold', borderRadius: 28, paddingTop: 15, paddingBottom: 15 }}>Log in</Text>
                  </TouchableOpacity>
                </LinearGradient>
              </View>
          ...
          <View style={{ position: 'absolute', bottom: 0, left:0, right:0 }}>
            <HideWithKeyboard>
              <View style={{ paddingBottom: 10, justifyContent: 'flex-end', height: width > height ? wp('15%') : wp('10%') }}>
                <Text style={{ textAlign: 'center', fontSize: width > height ? wp('1.3%') : wp('3.5%') }}>{Text_EN.Text_en.register} <Text style={{ color: '#01a2ff', textDecorationLine: 'underline' }} onPress={() => Linking.openURL('http://diwo.nu/')}>{Text_EN.Text_en.click_here}</Text></Text>
              </View>
            </HideWithKeyboard>
