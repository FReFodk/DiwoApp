...
import { Dimensions, StyleSheet, Text, View, ActivityIndicator, TouchableOpacity, Image, Alert, ScrollView, FlatList, Keyboard, TextInput } from 'react-native';
    ...
    fetch("http://diwo.nu/public/api/getSingleExperienceDetails", {
      ...
      .then((responseJson) => {
        ...
        if (responseJson.status == 200) {
          ...
          this.setState({ experience_date: responseJson.experience_details[0].date, experience_text_date: responseJson.experience_details[0].experience, experience_total_likes: responseJson.experience_details[0].total_likes, view_experience: true });
        }
      }).catch((error) => {
        console.error(error);
      });
      ...
      <View style={styles.container}>
        ...
        <Dialog
          ...
          <View style={{ position: 'relative' }}>
            <View style={styles.dialog_close_icon}>
              <TouchableOpacity onPress={() => this.setState({ view_experience: false })}>
                <Image
                  style={{
            ...
            <View style={{ paddingBottom: 10, marginTop: 50 }}>
              <View style={{ flexDirection: 'row' }}>
                ...
                <Text style={styles.dialog_txt}>{this.state.experience_date}</Text>
              </View>
              <View style={{ flexDirection: 'row', marginTop: 10 }}>
                <Text style={styles.dialog_txt_title}>Experience : </Text>
                <Text style={{ width: '70%', fontWeight: "500", color: "black", fontSize: width > height ? wp('2%') : wp('4%') }}>{this.state.experience_text_date}</Text>
              </View>
              <View style={{ flexDirection: 'row', marginTop: 10 }}>
                <Text style={styles.dialog_txt_title}>Total likes : </Text>
                <Text style={styles.dialog_txt}>{this.state.experience_total_likes}</Text>
              </View>
            </View>
          </View>
        </Dialog>
        ...
        <Dialog
          ...
          <View style={{ position: 'relative' }}>
            ...
            <View style={{ paddingBottom: 10, width: width > height ? wp('80%') : wp('68%') }}>
        ...
        <View style={{ flex: 1, padding: 15 }}>
          <ScrollView>
            ...
            <View style={styles.text_view}>
              ...
              />
              <Text style={{ textAlign: 'center', fontSize: width > height ? wp('1.5%') : wp('3.5%') }}>{Text_EN.Text_en.experience_note}</Text>
              ...
              <View style={styles.text_view}>
                ...
                <FlatList
                  ...
                  renderItem={({ item }) => <View style={styles.dynamic_list_view}>
                    <TouchableOpacity style={styles.list_part1} onPress={() => this.openExperience(item.id)}>
