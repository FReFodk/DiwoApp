...
import { Dimensions, StyleSheet, Text, View, TouchableOpacity, Image, Alert, TextInput, FlatList, ActivityIndicator } from 'react-native';
...
export default class home extends Component {
          ...
          fetch("http://diwo.nu/public/api/readSingleMessage", {
            ...
            .then((responseJson2) => {
              ...
              if (responseJson2.status == 200) {
                console.log(responseJson2);
                this.setState({ message_title: responseJson.message_details.title, message_text: responseJson.message_details.message, view_message_dialog: true });
                this.componentDidMount();
              } else {
                alert("Something went wrong");
              }
  ...
  reply_send = () => {
    ...
    if (this.state.message_title.length > 0 && this.state.message_text.length > 0) {
      const user_details = this.state.token;
      var headers = new Headers();
      let auth = 'Bearer ' + user_details.token;
      headers.append("Authorization", auth);
      ...
      fetch("http://diwo.nu/public/api/sendMessage", {
        ...
        .then((responseJson) => {
          ...
          if (responseJson.status == 200) {
            console.log(responseJson);
            this.setState({ title: "", message_text: "", replyMessage_dialog: false });
            this.componentDidMount();
          } else {
            alert("Something went wrong.");
          }
    ...
    if (rec_id.length > 0 && this.state.message_title.length > 0 && this.state.message_text.length > 0) {
      ...
      fetch("http://diwo.nu/public/api/sendMessage", {
        ...
        .then((responseJson) => {
          ...
          if (responseJson.status == 200) {
            console.log(responseJson);
            this.setState({ message_dialog: false, message_title: "", message_text: "", selectedItems: [], replyMessage_dialog: false });
            this.componentDidMount();
          } else {
            alert("Something went wrong.");
          }
    ...
    return (
      ...
      <View style={styles.container}>
        ...
        <Dialog
          ...
          onTouchOutside={() => this.setState({ replyMessage_dialog: false, errorText: false, message_title: "", message_text: "", selectedItems: [] })} >
          <View style={{ position: 'relative', padding: 15 }}>
            <View style={styles.dialog_close_icon}>
              <TouchableOpacity onPress={() => this.setState({ replyMessage_dialog: false, errorText: false, message_title: "", message_text: "", selectedItems: [] })}>
                <Image
                  style={{
            ...
            <View>
              <TouchableOpacity style={{ backgroundColor: '#00a1ff', padding: 10, paddingRight: 25, paddingLeft: 25, borderRadius: 5 }} onPress={() => this.reply_send()}>
        ...
        <Dialog
          ...
          onTouchOutside={() => this.setState({ view_message_dialog: false, errorText: false, message_title: "", message_text: "", selectedItems: [] })} >
          <View style={{ position: 'relative' }}>
            <View style={styles.dialog_close_icon}>
              <TouchableOpacity onPress={() => this.setState({ view_message_dialog: false, errorText: false, message_title: "", message_text: "", selectedItems: [] })}>
                <Image
                  style={{
                    width: width > height ? wp('3.5%') : wp('8%'),
                    height: width > height ? wp('3.5%') : wp('8%')
            ...
            <View style={{ paddingBottom: 10, marginTop: 50 }}>
              <View>
                ...
                <Text style={styles.dialog_txt}>{this.state.message_title}</Text>
              </View>
              <View style={{ marginTop: 10 }}>
                <Text style={styles.dialog_txt_title}>Message</Text>
                <Text style={styles.dialog_txt}>{this.state.message_text}</Text>
              </View>
            </View>
        ...
        <Dialog
          ...
          onTouchOutside={() => this.setState({ message_dialog: false, errorText: false, message_title: "", message_text: "", selectedItems: [] })} >
          <View style={{ position: 'relative', padding: 15 }}>
            <View style={styles.dialog_close_icon}>
              <TouchableOpacity onPress={() => this.setState({ message_dialog: false, errorText: false, message_title: "", message_text: "", selectedItems: [] })}>
                <Image
                  style={{
                    width: width > height ? wp('3.5%') : wp('8%'),
                    height: width > height ? wp('3.5%') : wp('8%')
        ...
        <View style={{ flex: 1, marginTop: 10 }}>
          <View style={{ flex: 0.99, }}>
            ...
            <View style={{ flexDirection: 'row', margin: 20 }}>
              ...
              <TouchableOpacity style={{ flex: 0.5, justifyContent: 'center', alignContent: 'center', alignItems: 'center', borderWidth: 1, borderRadius: 25, borderColor: 'lightgrey', marginRight: 5, padding: 8 }} onPress={() => { this.setState({ receiveMessage: 1, unreadMessage: 0, sendMessage: 0 }) }}>
                <View>
                  {this.state.receiveMessage == 1 ? <Text style={{ color: '#00a1ff', fontWeight: 'bold', fontSize: width > height ? wp('1.5%') : wp('4%') }}>{Text_EN.Text_en.Receive}</Text> : <Text style={{ fontSize: width > height ? wp('1.5%') : wp('3.5%') }}>{Text_EN.Text_en.Receive}</Text>}
                </View>
              </TouchableOpacity>
              <TouchableOpacity style={{ flex: 0.5, justifyContent: 'center', alignContent: 'center', alignItems: 'center', borderWidth: 1, borderRadius: 25, borderColor: 'lightgrey', marginLeft: 5, marginRight: 5, padding: 8 }} onPress={() => { this.setState({ receiveMessage: 0, unreadMessage: 0, sendMessage: 1 }) }}>
                <View>
                  {this.state.sendMessage == 1 ? <Text style={{ color: '#00a1ff', fontWeight: 'bold', fontSize: width > height ? wp('1.5%') : wp('4%') }}>{Text_EN.Text_en.Send}</Text> : <Text style={{ fontSize: width > height ? wp('1.5%') : wp('3.5%') }}>{Text_EN.Text_en.Send}</Text>}
                </View>
              </TouchableOpacity>
            </View>
            ...
            <ScrollView>
              <View style={{ padding: 20, marginTop: 10 }}>
                <FlatList
                  ...
                  renderItem={({ item }) =>
                    this.state.receiveMessage == 1 && item.receiver_id == this.state.userId ?
                      <View style={styles.dynamic_list_view}>
                        <View style={{ flexDirection: 'row', flex: 1 }}>
                          {this.state.userId == item.receiver_id ?
                            <TouchableOpacity style={styles.list_part1} onPress={() => this.messageView(item.id)}>
                              {item.is_read == 0 && item.receiver_id == this.state.userId ? <View style={styles.messageIndicator}><Text></Text></View> : <View style={styles.readIndicator}><Text></Text></View>}
                              <Text style={{ fontSize: width > height ? wp('2%') : wp('3.8%') }}>
                                {item.sender_name}{'\n'}
                          ...
                          <View style={styles.list_part2}>
                            ...
                            <TouchableOpacity onPress={() => this.messageReply(item.sender_id)}>
                              <Image
                                style={styles.like_icon}
                      ...
                      </View> : this.state.sendMessage == 1 && item.sender_id == this.state.userId ?
                        <View style={styles.dynamic_list_view}>
                          <View style={{ flexDirection: 'row', flex: 1 }}>
                            {this.state.userId == item.receiver_id ?
                              <TouchableOpacity style={styles.list_part1} onPress={() => this.messageView(item.id)}>
